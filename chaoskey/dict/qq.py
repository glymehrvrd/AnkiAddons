...
from comm.utils import jinjaEnv
...
<div style="font-size: 12pt">【基本解释】</div>
<br />
...
<div>
        <div style="color: #4d00cc; font-size: 12pt"><u>例句{% if se.p %}({{se.p}}){% endif %}:</u></div>
        {% for s in se.s %}
...
<!-- 相关词组 -->
{% for p in lo.ph %}
{% if loop.first %}<br /><div style="font-size: 10.5pt">相关词组</div>{% endif %}
...
{% if loop.first %}<br /><div style="font-size: 12pt">【情景会话】</div>{% endif %}
<br />
<div style="color: #4d00cc; font-size: 9pt"><b>{{dl.t}}</b></div>
{% for c in dl.c %}
<div>{{c.n}}:{{c.es}}</div>
<div>&nbsp;&nbsp;&nbsp;&nbsp;{{c.cs}}</div>
{% endfor %}
{% endfor %}
                ...
                self.mappingTemplate={u"单词":jinjaEnv.from_string(u"{{lo.word}}"),
                                      ...
                                      u"对话":jinjaEnv.from_string(u"{% for dl in dlg if dlg %}{% if loop.first %}<br /><div style=\"font-size: 12pt\">【情景会话】</div>{% endif %}<div style=\"color: #4d00cc; font-size: 9pt\"><b>{{dl.t}}</b></div>{% for c in dl.c %}<div>{{c.n}}:{{c.es}}</div><div>&nbsp;&nbsp;&nbsp;&nbsp;{{c.cs}}</div>{% endfor %}{% endfor %}")}
                # 显示字段的排序
                self.fieldSort=[u"单词",u"音标",u"解释",u"例句",u"变形词",u"同义词",u"反义词",u"词组",u"对话"]
                # 保存查询结果
                self.data={}
        ...
        def query(self,word,forceQuery=False):
                ...
                if not forceQuery:
                        js=utils.getDictByKey("dict_query_%s_%s"%(engine,word))
                        if js:
                ...
                try:
                        ...
                        if res.status==200:
                                ...
                                utils.setDictByKey("dict_query_%s_%s"%(engine,word),json.dumps(self.data))
                                ok=True
                except:
                        pass
                finally:
