...
{% if loop.first %}<br /><div><div style="color: #4d00cc; font-size: 12pt"><u>例句:</u></div>{% endif %}
        <div style="color: #009999">&nbsp;&nbsp;&nbsp;&nbsp;{{s.orig}}</div>
...
{% endif %}
        ...
        def query(self,word,forceQuery=False):
                ...
                if not forceQuery:
                        js=utils.getDictByKey("dict_query_%s_%s"%(self.engine,word))
                        if js:
                                self.data=json.loads(js)
                                return
                # ------从爱词霸网站上查询------
                conn = httplib.HTTPConnection('dict-co.iciba.com')
                params = urllib.urlencode({'w': word})
                headers = {"Content-type": "application/x-www-form-urlencoded","Accept": "text/plain"}
                ok=False
                try:
                        conn.request("POST", "/api/dictionary.php",params, headers) 
                        res = conn.getresponse()
                        if res.status==200:
                                # ------提取XML数据------
                                self.data=utils.xml2json(parseString(res.read()).documentElement)
                                utils.setDictByKey("dict_query_%s_%s"%(self.engine,word),json.dumps(self.data))
                                ok=True
                except:
                        pass
                finally:
