...
# 数据库文件
datafile = os.path.join(curpath, "onlinedict.db").decode(sys.getfilesystemencoding())
...
def getConfByKey(key):
        r=getCursor().execute("SELECT value FROM conf WHERE key=?",(key,)).fetchone()
        if r :
            return r['value']
        return None
def setConfByKey(key,value):
        if getConfByKey(key):
            getCursor().execute("UPDATE conf SET value=? WHERE key=?",(value,key))
        else:
            getCursor().execute("INSERT INTO conf(key,value) VALUES(?,?)",(key,value))

# 词典数据读取
def getDictByKey(key):
        r=getCursor().execute("SELECT value FROM dict WHERE key=?",(key,)).fetchone()
        if r :
            return r['value']
        return None
...
def setDictByKey(key,value):
        if getDictByKey(key):
                getCursor().execute("UPDATE dict SET value=? WHERE key=?",(value,key))
        else:
                getCursor().execute("INSERT INTO dict(key,value) VALUES(?,?)",(key,value))
